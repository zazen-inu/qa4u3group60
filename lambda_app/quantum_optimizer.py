...
def optimize_ingredients(ingredients_df, target_nutrition):
    ...
    注: 実際の量子アニーリングライブラリ（dimod, neal）がない場合は
    シンプルな最適化アルゴリズムで代用
    """
    try:
        from neal import SimulatedAnnealingSampler as SASampler
        from dimod import BinaryQuadraticModel
        return _optimize_with_quantum(ingredients_df, target_nutrition)
    except ImportError:
        return _optimize_with_simple_algorithm(ingredients_df, target_nutrition)

def _optimize_with_quantum(ingredients_df, target_nutrition):
    """量子アニーリングを使用した最適化（dimod, nealライブラリ使用）"""
    from neal import SimulatedAnnealingSampler as SASampler
    from dimod import BinaryQuadraticModel
    
    num_ingredients = len(ingredients_df)
    if num_ingredients == 0:
        return {"error": "原材料が見つかりません"}
    ...
    target_calories_g = target_calories / total_weight if target_calories > 0 else 1
    target_protein_g = target_protein / total_weight if target_protein > 0 else 1
    target_fat_g = target_fat / total_weight if target_fat > 0 else 1
    target_carbs_g = target_carbs / total_weight if target_carbs > 0 else 1
    ...
    num_bits_per_ingredient = 8  # 計算量削減のため8ビットに
...
def _optimize_with_simple_algorithm(ingredients_df, target_nutrition):
    """シンプルな最適化アルゴリズム（量子アニーリングライブラリがない場合）"""
    num_ingredients = len(ingredients_df)
    if num_ingredients == 0:
        return {"error": "原材料が見つかりません"}
    ...
    target_fat = target_nutrition.get('fat', 0)
    target_carbs = target_nutrition.get('carbs', 0)
    target_salt = target_nutrition.get('salt', 0)
    ...
    if total_ingredients == 1:
        proportions[ingredients[0]['name']] = 100.0
    elif total_ingredients == 2:
        proportions[ingredients[0]['name']] = 60.0
        proportions[ingredients[1]['name']] = 40.0
    elif total_ingredients == 3:
        proportions[ingredients[0]['name']] = 50.0
        proportions[ingredients[1]['name']] = 30.0
        proportions[ingredients[2]['name']] = 20.0
    else:
        proportions[ingredients[0]['name']] = 50.0
        proportions[ingredients[1]['name']] = 25.0
        proportions[ingredients[2]['name']] = 15.0
        
        remaining = 10.0
        for i in range(3, total_ingredients):
            proportions[ingredients[i]['name']] = remaining / (total_ingredients - 3)
    ...
    for ingredient in ingredients:
        proportion = proportions[ingredient['name']] / 100
        for nutrient in ['calories', 'protein', 'fat', 'carbs', 'salt']:
            calculated_nutrition[nutrient] += ingredient[nutrient] * proportion
    
    result = {
        'proportions': proportions,
        'calculated_nutrition': {
            'per_gram': {
                nutrient: value / 100 for nutrient, value in calculated_nutrition.items()
            },
            'total': {
                nutrient: value * total_weight / 100 for nutrient, value in calculated_nutrition.items()
            }
        }
    }
